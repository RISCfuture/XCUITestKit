...
- `clearAndType(_:app:perCharacter:)`: a `perCharacter` flag types one character at a time, for fields backed by a self-canonicalizing formatter (e.g. a fractional/decimal-shift `TextField(value:format:)`) where a single `typeText` burst can race the format round-trip and corrupt the buffer.
