...
| `XCUIElement` interaction | `forceTap`, `tapStable` / `coordinateTapWhenFrameStable` (frame-stability polling to dodge "Activation point invalid" and Liquid Glass hittability), `isVisible`, `isOn` (normalizes a toggle/switch/checkbox's `Int`/`String`/`Bool` value bridging), `makeVisible` (reveal an element inside a known scroll container), `commitByMovingFocus` (end editing on a Return-less `numberPad`/`decimalPad` field by focusing a sibling). |
